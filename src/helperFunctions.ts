...
export function parseEmails(
  therapist: Therapist,
  emails = therapist.email!.split(",")
) {
  return emails
    .map((email) => parseEmail(therapist, email))
    .filter((e) => emailRegex.test(e));
}

export function parseEmail(therapist: Therapist, email: string) {
  return email
    .replace(/etunimi|etu-nimi/, therapist.firstName)
    .replace(/sukunimi|suku-nimi/, therapist.lastName)
    .replace("(at)", "@")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
...
export const parseTableEmails = (table: MRT_TableInstance<Therapist>) => {
  const emails = table.getSelectedRowModel().flatRows.map((row) => {
    const therapist = row.original;
    const parsedEmails = parseEmails(therapist);
    return parsedEmails;
  });
...
export function sendEmail(table: MRT_TableInstance<Therapist>) {
  const filteredEmails = parseTableEmails(table);
  let mail = document.createElement("a");
  mail.href = `mailto:?bcc=${filteredEmails.join(",")}`;
  mail.target = "_blank";
  mail.click();
}
export function copyEmails(table: MRT_TableInstance<Therapist>) {
  const filteredEmails = parseTableEmails(table);
  const joined = filteredEmails.join(",");
  navigator.clipboard.writeText(joined);
  console.log(joined);
}

export function isSelected(table: MRT_TableInstance<Therapist>) {
